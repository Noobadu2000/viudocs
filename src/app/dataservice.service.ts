...
export class DataserviceService {
  ...
  signin = (email: string, password: string, callback: any) => {
    ...
    return this.angfireAuth.auth.signInWithEmailAndPassword(email, password)
      .then((result: any) => {
        this.postloginData(true);
        return callback({ error: false, response: result });
      }).catch((error: any) => {
        return callback({ error: true, message: error.message });
      });
  }
  verfiersignin(email, password, callback) {
    // this.angfireAuth.auth
    //   .signInWithEmailAndPassword(email, password)
    //   .then((res) => {
    //     this.router.navigate(["/"]);
  ...
  signup(email: string, password: string, firstName: string, lastName: string, callback: any) {
    this.angfireAuth.auth
      ...
      .then((res) => {
        ...
        return callback({ error: false, message: 'You have been successfully registered to ViuDocs' });
